...
import com.camp.inventory.ContainerNetherBlock;
...
public class NetherBeaconGui extends GuiContainer
{
    private static final Logger logger = LogManager.getLogger();
    private static final ResourceLocation beaconGuiTextures = new ResourceLocation(cm.MODID+":"+"textures/gui/container/beacon.png");
    private IInventory tileNetherBlock;
    private NetherBeaconGui.ConfirmButton netherBlockConfirmButton;
    private boolean buttonsNotDrawn;
    private static final String __OBFID = "CL_00000739";

    public NetherBeaconGui(InventoryPlayer p_i45507_1_, IInventory p_i45507_2_)
    {
        super(new ContainerNetherBlock(p_i45507_1_, p_i45507_2_));
        this.tileNetherBlock = p_i45507_2_;
        this.xSize = 230;
        this.ySize = 219;
    }
    ...
    {
        super.initGui();
        this.buttonList.add(this.netherBlockConfirmButton = new NetherBeaconGui.ConfirmButton(-1, this.guiLeft + 164, this.guiTop + 107));
        this.buttonList.add(new NetherBeaconGui.CancelButton(-2, this.guiLeft + 190, this.guiTop + 107));
        this.buttonsNotDrawn = true;
        this.netherBlockConfirmButton.enabled = false;
    }
    ...
    {
        super.updateScreen();
        int i = this.tileNetherBlock.getField(0);
        int j = this.tileNetherBlock.getField(1);
        int k = this.tileNetherBlock.getField(2);
        ...
        if (this.buttonsNotDrawn && i >= 0)
        {
            ...
            int k1;
            int l1;
            NetherBeaconGui.PowerButton powerbutton;

            for (int l = 0; l <= 2; ++l)
            {
                i1 = TileEntityNetherBlock.effectsList[l].length;
                j1 = i1 * 22 + (i1 - 1) * 2;

                for (k1 = 0; k1 < i1; ++k1)
                {
                    l1 = TileEntityNetherBlock.effectsList[l][k1].id;
                    powerbutton = new NetherBeaconGui.PowerButton(l << 8 | l1, this.guiLeft + 76 + k1 * 24 - j1 / 2, this.guiTop + 22 + l * 25, l1, l);
            ...
            byte b0 = 3;
            i1 = TileEntityNetherBlock.effectsList[b0].length + 1;
            j1 = i1 * 22 + (i1 - 1) * 2;

            for (k1 = 0; k1 < i1 - 1; ++k1)
            {
                l1 = TileEntityNetherBlock.effectsList[b0][k1].id;
                powerbutton = new NetherBeaconGui.PowerButton(b0 << 8 | l1, this.guiLeft + 167 + k1 * 24 - j1 / 2, this.guiTop + 47, l1, b0);
                this.buttonList.add(powerbutton);
            ...
            if (j > 0)
            {
                NetherBeaconGui.PowerButton powerbutton1 = new NetherBeaconGui.PowerButton(b0 << 8 | j, this.guiLeft + 167 + (i1 - 1) * 24 - j1 / 2, this.guiTop + 47, j, b0);
        ...
        this.netherBlockConfirmButton.enabled = this.tileNetherBlock.getStackInSlot(0) != null && j > 0;
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        {
            String s = "CM|NetherBlock";
            PacketBuffer packetbuffer = new PacketBuffer(Unpooled.buffer());
            packetbuffer.writeInt(this.tileNetherBlock.getField(1));
            packetbuffer.writeInt(this.tileNetherBlock.getField(2));
            this.mc.getNetHandler().addToSendQueue(new C17PacketCustomPayload(s, packetbuffer));
            this.mc.displayGuiScreen((GuiScreen)null);
        }
        else if (button instanceof NetherBeaconGui.PowerButton)
        {
            if (((NetherBeaconGui.PowerButton)button).func_146141_c())
            {
                return;
            }
            ...
            if (i < 3)
            {
                this.tileNetherBlock.setField(1, k);
            }
            else
            {
                this.tileNetherBlock.setField(2, k);
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY)
    {
        ...
        this.itemRender.renderItemAndEffectIntoGUI(new ItemStack(ItemManager.americanIngot), k + 42 - 44, l + 109);
        this.itemRender.renderItemAndEffectIntoGUI(new ItemStack(Items.quartz), k + 42 - 22, l + 109);
    ...
    static class Button extends GuiButton
        {
            ...
            public void drawButton(Minecraft mc, int mouseX, int mouseY)
            {
                if (this.visible)
                {
                    mc.getTextureManager().bindTexture(NetherBeaconGui.beaconGuiTextures);
                    GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                    ...
                    if (!NetherBeaconGui.beaconGuiTextures.equals(this.field_146145_o))
                    {
    ...
    @SideOnly(Side.CLIENT)
    class CancelButton extends NetherBeaconGui.Button
    {
        private static final String __OBFID = "CL_00000740";

        public CancelButton(int p_i1074_2_, int p_i1074_3_, int p_i1074_4_)
        {
            super(p_i1074_2_, p_i1074_3_, p_i1074_4_, NetherBeaconGui.beaconGuiTextures, 112, 220);
        }

        public void drawButtonForegroundLayer(int mouseX, int mouseY)
        {
            NetherBeaconGui.this.drawCreativeTabHoveringText(I18n.format("gui.cancel", new Object[0]), mouseX, mouseY);
        }
    }

    @SideOnly(Side.CLIENT)
    class ConfirmButton extends NetherBeaconGui.Button
    {
        private static final String __OBFID = "CL_00000741";

        public ConfirmButton(int p_i1075_2_, int p_i1075_3_, int p_i1075_4_)
        {
            super(p_i1075_2_, p_i1075_3_, p_i1075_4_, NetherBeaconGui.beaconGuiTextures, 90, 220);
        }

        public void drawButtonForegroundLayer(int mouseX, int mouseY)
        {
            NetherBeaconGui.this.drawCreativeTabHoveringText(I18n.format("gui.done", new Object[0]), mouseX, mouseY);
        }
    }

    @SideOnly(Side.CLIENT)
    class PowerButton extends NetherBeaconGui.Button
    {
        ...
        public void drawButtonForegroundLayer(int mouseX, int mouseY)
        {
            ...
            NetherBeaconGui.this.drawCreativeTabHoveringText(s, mouseX, mouseY);
        }
    }
}